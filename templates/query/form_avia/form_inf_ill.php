...
<!--<div class="col-lg-12">
    <br> <br> <br>
    <center> <b>Информация по больничным</b> </center>
</div>-->

<!--форма-->

<form  role="form" id="formFillCar" method="POST" action="/str/builder/basic/inf_ill/4#result_page">
    ...
    <div class="row">
        ...
        <div class="col-lg-2">
            <div class="form-group">
                <label for="maim">Вид травмы</label>
                <select class="form-control" name="maim" id="maim" >

                    <option value="" selected="">все</option>
                    <?php
                    foreach ($maim as $m) {
                        if (isset($_POST['maim']) && ($_POST['maim'] == $m['id'])) {
                            printf("<p><option value='%s' selected ><label>%s</label></option></p>", $m['id'], $m['name']);
                        }
                        printf("<p><option value='%s' ><label>%s</label></option></p>", $m['id'], $m['name']);
                    }
                    ?>
