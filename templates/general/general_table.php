    ...
    <div role="tabpanel" class="tab-pane active" id="general_table">
        ...
        <div class="col-lg-12">
            ...
            if (date(" H:i:s") < '00:00:00') {
                ?>
                <center> <u><b>Строевая записка ОПЧС c 08:00 <?= date("d.m.Y") ?> до 08:00  <?= date("d.m.Y", time() + (60 * 60 * 24)) ?>  </b></u></center>
                <?php
            } else {
                ?>
                <center> <u><b>Строевая записка ОПЧС c 08:00 <?= date("d.m.Y", time() - (60 * 60 * 24)) ?> до 08:00  <?= date("d.m.Y") ?>  </b></u></center>
                <?php
            }
            ?>
...
<!--                        <th>Информация</th>-->
                        ...
                        if ((($_SESSION['ulevel'] == 1) || ($_SESSION['ulevel'] == 2) || ($_SESSION['ulevel'] == 3)) && ($_SESSION['is_admin'])) {//РЦУ может открыть доступ на ред.заступившей смене либо область
                            ...
                            <th>Открыть/закрыть<br>доступ<br>
                                (доступно до <?= date('H:i', strtotime($time_allow_open))?>)</th>
                            <th>Доступ открыл</th>
                            <?php
                        }
